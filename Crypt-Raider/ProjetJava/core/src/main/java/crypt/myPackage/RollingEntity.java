...
public abstract class RollingEntity extends GameObject{
    protected static final int TILE_SIZE = 80; // Taille d'une tuile
    protected boolean isFalling = false;      // Indique si la boule est en train de tomber
    protected float fallSpeed = 200f;         // Vitesse de chute (pixels par seconde)
    protected float fallProgress = 0;         // Progression actuelle dans la chute
    protected boolean hasExploded = false;    // Évite les explosions multiples
    protected int SCREEN_WIDTH = 1200;
    protected int SCREEN_HEIGHT = 720;
    public RollingEntity(Texture texture, float x, float y) {
        super(texture, x, y);
    ...
    public void explode(List<GameObject> gameObjects, List<GameObject> objectsToRemoveGlobal) {
            ...
            for (GameObject obj : gameObjects) {
                if (obj.isActive() && explosionArea.overlaps(obj.getBounds())) {
                    if (!(obj instanceof Joueur) &&/* || obj instanceof Enemie || */obj.isDestructible()  ) {
                        ...
                        System.out.println("Désactivation de l'objet : " + obj.getClass().getSimpleName());
                        obj.deactivate();
                        ((Bombe) obj).explode(gameObjects, objectsToRemoveGlobal);
                    ...
                    System.out.println("n'est pas destructible "+ obj.getClass().getSimpleName());
                }
            }
        }
